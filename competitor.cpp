...
struct MyState
{
  MyState(trader_id_t trader_id) : trader_id(trader_id), books(), submitted(), open_orders(),
...
public:
  ...
  void on_order_update(Common::OrderUpdate &update, Bot::Communicator &com)
  {
    ...
    // spread size
    price_t spread_size = best_offer - best_bid;
    ...
    if (best_bid != 0.0 && spread_size >= bid_edge + ask_edge + 2 * edge_range)
    { // 0.0 denotes no bid
  ...
  { /*
    if (trade_with_me_in_this_packet)
    {
      ...
      std::cout << "got trade with me; " << trader_id << "pnl = "
